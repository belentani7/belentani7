...
export default function LoreExplorer() {
  ...
  const filtered =
    selectedCategory === "All"
      ? LORE_DATABASE
      : LORE_DATABASE.filter(n => n.category === selectedCategory);
      ...
      >
        <div className="flex flex-col justify-between gap-10 border-b border-[var(--border)] pb-8 md:flex-row md:items-end">
          <div className="fade-up">
            ...
            <h1 className="section-title mt-7 max-w-[580px]">
              The complete mythology of Belentani.
            </h1>
          </div>
          <p className="mono-copy max-w-[320px] fade-up delay-1">
            Este archivo contiene el corpus mitológico completo. Cada nodo
            documenta una capa de la Dimensión Zion, sus personajes y sus eras.
          </p>
        </div>
          ...
          <div className="space-y-4">
            {filtered.map(node => (
              ...
              >
                ...
                <p className="mt-2 text-sm leading-6 text-[#96918a] line-clamp-2">
                  {node.summary}
                </p>
              </button>
            ))}
          ...
          <div className="border border-[var(--border)] bg-[#101010] p-8 sm:p-10">
            <div className="flex items-center justify-between border-b border-[var(--border)] pb-4">
              <span className="archive-label">
                {activeNode.category} / Node ID: {activeNode.id}
              </span>
              <div className="flex gap-1.5">
                {activeNode.tags.map(tag => (
                  <span
                    key={tag}
                    className="border border-[var(--border)] px-2 py-0.5 font-mono text-[10px] text-[#96918a]"
                  >
                    #{tag}
                  </span>
                ))}
