    ...
    const animate = () => {
      ...
      for (let y = 0; y < canvas.height; y += 40) {
        ...
        for (let x = 0; x < canvas.width; x += 20) {
          const wave =
            Math.sin((x + time * 50) * 0.01) * 15 +
            Math.cos((y + time * 30) * 0.01) * 15;
          const yPos = y + wave;
...
export function CollapseShader({ active = false }: { active?: boolean }) {
  ...
  return (
    <canvas
      ref={canvasRef}
      className="fixed inset-0 pointer-events-none opacity-50 z-15"
    />
  );
}
...
export function ParticleBloom() {
  ...
  return (
    <canvas
      ref={canvasRef}
      className="fixed inset-0 pointer-events-none opacity-35 z-5"
    />
  );
}
